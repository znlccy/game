...
public interface UserActiveMapper {
    ...
    @Select("SELECT      \n" +
            ...
            "   GROUP BY userLoginTime) AS a WHERE a.StatisticsDate>=DATE_FORMAT(CONCAT(#{statisticsRequest.beginTime},' 00:00:00'),'%Y-%m-%d %H:%i:%s') && a.StatisticsDate<=DATE_FORMAT(CONCAT(#{statisticsRequest.endTime},' 23:59:59'),'%Y-%m-%d %H:%i:%s')\n" +
